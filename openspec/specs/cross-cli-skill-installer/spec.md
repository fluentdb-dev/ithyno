...
> ⚠️ **PENDING MODIFIED** by [copy-claude-ithy-opsx-into-agents-workflows-for-agy](../../changes/copy-claude-ithy-opsx-into-agents-workflows-for-agy/): antigravity install gains a second helper that COPIES `.claude/commands/ithy-opsx/*.md` into `.agents/workflows/ithy-opsx/*.md` (non-destructive; complements the existing MOVE migration); adds `kind: "move" | "copy"` field on migration entries plus three new scenarios (copy, copy-skip-on-conflict, copy-not-invoked-without-antigravity).
