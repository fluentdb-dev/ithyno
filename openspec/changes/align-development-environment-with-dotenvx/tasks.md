...
- [ ] 1.1 Add temporary-project integration fixtures that use the bundled dotenvx to encrypt a plaintext `.env` and `.env.development`, generate `.env.keys`, and resolve the original values.
- [ ] 1.2 Add failing regression tests proving the current parser-only resolver returns ciphertext, first encryption is incorrectly blocked without a pre-existing key, and suffixed `DOTENV_PRIVATE_KEY_*` credentials are not modelled safely.
...
- [ ] 2.1 Implement one dotenvx adapter that resolves the ordered base/selected profiles into an isolated object using the project-root `.env.keys` path and normalized secret-safe errors.
- [ ] 2.2 Separate syntax/source metadata from resolved runtime values so ciphertext is never returned as an application value after decryption failure.
- [ ] 2.3 Replace the fixed key allowlist with profile-aware `DOTENV_PRIVATE_KEY` family handling plus explicitly supported legacy credentials.
...
- [ ] 3.1 Change first-time encryption to invoke bundled dotenvx with explicit project-root `-f` and `-fk` paths without requiring a preconfigured private key.
...
- [ ] 3.3 Refuse unsafe or Git-tracked key files and preserve pre-operation files/state when dotenvx encryption fails.
