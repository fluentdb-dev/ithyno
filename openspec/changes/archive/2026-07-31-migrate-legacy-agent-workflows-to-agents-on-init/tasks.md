...
- [x] 1.1 Create `server/skill-renderer/migrate-agy.ts` exporting
...
- [x] 1.2 Extend `InstallResult` in `server/skill-renderer/types.ts`
      with `migrations: Array<{ cli: CliId, moved: string[], skipped: Array<{ path: string, reason: string }> }>`.
- [x] 1.3 Wire the migration into `installSkills` in
...
- [x] 2.1 Unit tests for `migrateLegacyAntigravityDir`: (a) moves
...
- [x] 2.2 installSkills e2e: after seeding a fixture project with
...
- [x] 2.3 installSkills e2e — conflict case: seed both
...
- [x] 3.1 `npm run openspec -- validate migrate-legacy-agent-workflows-to-agents-on-init --strict` — passes.
- [x] 3.2 `npm run typecheck` — clean.
- [x] 3.3 `npm test` — all pass; new migration tests green.
...
- [x] 4.1 Write `openspec/changes/migrate-legacy-agent-workflows-to-agents-on-init/outcome.md`
