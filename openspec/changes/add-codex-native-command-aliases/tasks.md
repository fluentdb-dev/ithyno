...
- [ ] 1.3 Run upstream OpenSpec initialization with a project-local Codex home
  and rename generated `opsx-*.md` prompts to `openspec-*.md` safely.

## 2. Configure Codex Manager startup

- [x] 2.1 Thread the absolute project-local `CODEX_HOME` into the Codex
  Manager PTY environment without changing other Manager environments.
- [ ] 2.2 Add initialization tests proving global Codex prompt paths are not
...
- [ ] 4.1 Add a target-agent CLI + role command resolver for Manager, code,
  review, and verify delivery.
- [ ] 4.2 Use the resolver in direct subprocess and agmsg worker prompt
...
- [ ] 4.4 Add role-by-role Codex and non-Codex dispatch regression tests.

## 5. Verify live compatibility

- [ ] 5.1 Add an executable Codex prompt-discovery smoke harness using the
  installed supported CLI version and the exact user-facing invocation.
- [ ] 5.2 Keep Codex marked `(unverified)` unless the smoke harness passes.
- [ ] 5.3 Run `npm run typecheck && npm test && npm run build`.
- [x] 5.4 Run `npm run openspec -- validate add-codex-native-command-aliases --strict`.
- [ ] 5.5 Write `outcome.md` before archive.
