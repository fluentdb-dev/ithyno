...
- [ ] 3.2 Manual smoke test on Windows: window appears within ~300 ms of launch (welcome page visible while server starts), then navigates to app
- [ ] 3.3 Manual smoke test: "File → Open Project…" still works (teardown → spawn → navigate)
- [ ] 3.4 Manual smoke test: healthy-session reuse path unchanged (no welcome flash on reload)
