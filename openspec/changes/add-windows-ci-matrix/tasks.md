...
- [ ] 1.1 Create `.github/workflows/test.yml` with `name: Test`,
      triggers `pull_request` + `push: branches: [develop, main]` +
      `workflow_dispatch`.
- [ ] 1.2 Set `concurrency: group: test-${{ github.ref }}` with
      `cancel-in-progress: true` (distinct group from `release.yml`'s
      `release-${{ github.ref }}`).
- [ ] 1.3 Define one job `test` with `runs-on: ${{ matrix.os }}` and
      `strategy: matrix: os: [macos-latest, windows-latest,
      ubuntu-latest]` + `fail-fast: false`.
- [ ] 1.4 Set `defaults.run.shell: bash` at the job level so every
...
- [ ] 1.6 Do NOT add an `actions/upload-artifact@v4` step —
      `release.yml` owns artifact production.
- [ ] 1.7 Do NOT reference `secrets.*` beyond the default
      `GITHUB_TOKEN` (which is auto-provisioned and not called out).

## 2. Line-ending policy

- [ ] 2.1 Create root `.gitattributes` declaring `* text=auto eol=lf`.
- [ ] 2.2 Add `-text` exceptions for known binary patterns: `*.png`,
      `*.jpg`, `*.jpeg`, `*.gif`, `*.ico`, `*.icns`, `*.pdf`, `*.woff*`,
      `*.ttf`, `*.otf`, `*.dmg`, `*.exe`, `*.vsix`, `*.tgz`, `*.zip`,
      `*.node` (native modules).
- [ ] 2.3 Run `git add --renormalize .` locally after landing
...
- [ ] 2.4 Verify no *intentionally-CRLF* file exists in the repo (grep
...
- [ ] 3.1 Grep for hard-coded `"/"` string joins in `server/**.ts` and
      `web/src/**` tests: `grep -rn "'/'" server/ web/src/ | grep -v
      "://" | grep -E "path|join|resolve"`.
- [ ] 3.2 For each hit, decide: is this a POSIX-only path (safe to
      leave), or a filesystem path that should use `path.sep` /
      `path.join()`? Fix in place if the latter.
- [ ] 3.3 Do NOT proactively refactor non-test source code unless a
      Windows CI run reveals a concrete regression. This audit is
      scoped to tests only.

## 4. Verification — pre-merge (dev machine)

- [ ] 4.1 `npm run openspec -- validate add-windows-ci-matrix --strict`
      passes.
- [ ] 4.2 `npm test` on macOS still passes after `.gitattributes` and
      any test-file edits (baseline).
...
- [ ] 6.1 Do NOT modify `release.yml`. Its matrix is already correct
      for release-build; changing it is out of scope.
- [ ] 6.2 Do NOT automate the "no `[install-skills]` log line" Doctor
      sanity check. Remains Phase C manual. If a follow-up wants it,
      that is a separate proposal.
- [ ] 6.3 Do NOT add a Node-version matrix. Node 20 only.
- [ ] 6.4 Do NOT add arm64 runners. x64 only.

## 7. Outcome

- [ ] 7.1 Write `openspec/changes/add-windows-ci-matrix/outcome.md`
      capturing:
