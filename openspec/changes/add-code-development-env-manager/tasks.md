...
- [x] 1.1 Add and pin the dotenvx runtime dependency used by packaged CLI, Electron, and VSIX builds.
- [x] 1.2 Add `server/environment/` types and path guards for supported project-root `.env*` profiles, revisions, diagnostics, and selected-profile state.
- [x] 1.3 Implement the dotenvx adapter for masked parsing, ordered profile resolution, encryption/key-state inspection, and normalized errors.
- [x] 1.4 Add unit tests for profile discovery, precedence, invalid syntax, escaping symlinks, and reserved `ITHYNO_*` filtering.

## 2. Safe persistence and diagnostics

- [x] 2.1 Implement project-local selection persistence in `.ithyno/environment.json` without storing environment values.
- [x] 2.2 Implement revision-checked env mutations with key validation, compatible-line preservation, restrictive temporary-file permissions, and atomic rename.
- [x] 2.3 Implement secret-safe diagnostics for unreadable files, unsupported syntax, reserved keys, encryption/key state, and risky Git tracking.
- [x] 2.4 Add tests proving stale writes and path traversal are rejected and plaintext values do not appear in diagnostics or ordinary errors.
