...
- [ ] 1.1 In `server/agents/registry.ts`, add `maxReworkRounds: number` to the `AgentConfig` type/interface (top-level, sibling of `maxParallel`).
- [ ] 1.2 Add constants `DEFAULT_MAX_REWORK_ROUNDS = 5`, `MAX_REWORK_ROUNDS_MIN = 1`, `MAX_REWORK_ROUNDS_MAX = 10`.
- [ ] 1.3 Add `validateMaxReworkRounds(value: unknown): number` mirroring `validateMaxParallel`:
...
- [ ] 1.4 In `load()`, extract `maxReworkRounds` from the parsed YAML and pass through `validateMaxReworkRounds`. Store on the AgentConfig instance.
- [ ] 1.5 Ensure `publicConfig()` (the client-facing DTO) includes `maxReworkRounds`.

## 2. Registry tests

- [ ] 2.1 Extend `server/agents/registry.test.ts` with tests mirroring the `maxParallel` cases:
...
- [ ] 2.2 Update any fixture test files (e.g., `spawn-options-writer.test.ts`) whose `AgentConfig` mocks are missing the new field. Add `maxReworkRounds: 5` (or the default) to each fixture.

## 3. Client types

- [ ] 3.1 Add `maxReworkRounds: number` to `AgentConfigResponse` in `web/src/types.ts`.

## 4. Skill documentation

- [ ] 4.1 `.claude/commands/ithy-opsx/dispatch.md`:
...
- [ ] 4.2 `.claude/skills/ithy-opsx-dispatch-multi/SKILL.md`:
...
- [ ] 5.1 `npm test` passes after the registry + fixture updates.
...
- [ ] 6.1 `npm run openspec -- validate add-agents-max-rework-rounds-config --strict` passes.
- [ ] 6.2 `npm test` passes.
...
- [ ] 6.8 Write `openspec/changes/add-agents-max-rework-rounds-config/outcome.md` (✅ Worked / ⚠️ Surprises / 🔁 Differently / 🌱 Follow-ups). Follow-ups: consider per-role overrides (e.g., `agents[].maxReworkRounds` overriding the top-level); consider exposing the field in a UI settings panel.
