...
export async function toggleAgentHook(agentName: string, enabled: boolean, context?: "electron" | "vscode" | "cli", hostAppName?: string): Promise<void> {
  const { status, data } = await postJson<{ ok?: boolean; error?: string }>("/api/agent-hooks/toggle", { agentName, enabled, context, hostAppName });
  if (status >= 400) throw new Error(data.error ?? `HTTP ${status}`);
}
