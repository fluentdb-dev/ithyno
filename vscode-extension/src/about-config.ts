...
/**
 * VS Code extension mirror of server/about-config.ts.
 *
 * The VS Code extension compiles to its own isolated CommonJS bundle and
 * cannot import directly from server/ (separate rootDir). This file replicates
 * the same constants and derivation logic so sponsors and URL constants have a
 * single point of change PER SURFACE — and all surfaces produce identical
 * AboutInfo when built from the same package.json + constants.
 *
 * KEEP IN SYNC with:
 ...
 * Append new entries here — no further per-surface client code change needed.
...
/**
 * Build a complete AboutInfo from the extension's own package.json shape.
 * The extension manifest has version, license, and description but not
 * repository/bugs URLs — those fall back to REPO_URL constants.
 */
