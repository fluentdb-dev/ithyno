...
function dispatchExecutionRule(source: SkillSource): RenderedFile {
  const content = [
    ...
    "   session.",
    "2. **Resolve every selected worker.** For each change and each stage, read",
    "   the matching role entry from `agents.yaml` before choosing a launch",
    "   mechanism. Never replace a cross-CLI worker with an Agy subagent merely",
    "   to make multi-change fan-out easier.",
    "3. **Preserve routing priority.** A configured `live-shell` agmsg worker",
    ...
    "4. **Preserve the worker contract.** Set the tool's `TypeName` / `Role`",
    ...
    "5. **Do not bypass delegation.** Do not directly run `openspec-apply-change`, edit",
    ...
    "6. **Use only the injected dashboard endpoint.** Before dispatch, use",
    ...
    "7. **Question freshness before every request.** Ask whether the dashboard",
